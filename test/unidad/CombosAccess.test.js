...
describe('ComboAccessTest', () => {
    ...
    it('getData debería obtener todos los combos (o paginados)', async () => {
        ...
        fetchStub.resolves(new Response(JSON.stringify(mockCombos), {
            status: 200,
    ...
    it('getData con id debería obtener un combo específico', async () => {
        ...
        fetchStub.resolves(new Response(JSON.stringify(mockCombo), {
            status: 200,
            headers: { "Content-Type": "application/json" },
        }));
    ...
    it('updateData debería actualizar un combo', async () => {
        ...
        fetchStub.resolves(new Response(JSON.stringify(updatedComboData), {
            status: 200,
            headers: { "Content-Type": "application/json" },
        }));

        const resultResponse = await combo.updateData({ nombre: 'Combo 1 actualizado' }, 1); // Capitalización corregida
        const data = await resultResponse.json();
    ...
    it('createData debería crear un combo', async () => {
        ...
        fetchStub.resolves(new Response(JSON.stringify({ id: 999, ...newComboData }), {
    ...
    it('deleteData debería eliminar un combo', async () => {
        fetchStub.resolves(new Response(null, { // No hay body típicamente para DELETE
            status: 204 // 204 No Content es común para un DELETE exitoso
        }));
    ...
    it('getDataPorNombre debería obtener combos por nombre', async () => {
        ...
        fetchStub.resolves(new Response(JSON.stringify(mockCombosByName), {
            ok: true,
            status: 200,
            headers: { "Content-Type": "application/json" },
        }));
